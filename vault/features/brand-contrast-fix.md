...
`color.brand.primary` is `#4D9463`. White text on that surface lands at **~3.4:1 contrast** — passes WCAG 2.2 AA 1.4.11 (UI component ≥3:1) but **fails 1.4.3 Contrast (Minimum)** for normal text (≥4.5:1). Surfaced by `ui-designer` during the `reply-coach` dispatch (see `vault/features/reply-coach.md` `### Visual` "Flagged gaps" §2) and ratified by design-lead as out of scope for that feature because it's a system-wide brand-token concern, not a chip-specific gap.
...
- [ ] Axe sweep (per the project's existing a11y harness — see `vault/features/a11y-audit.md`) reports zero 1.4.3 failures across the 11 audited pages after migration.
