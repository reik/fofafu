...
## Review
- [ ] [[features/header-nav-redesign]] @design — collaborator-scope feature (owner=engineering; design added at dispatch routing), so no In Progress card was ever created; added directly to Review. ui-designer + a11y-auditor returned; Visual complete (NavTrack/NavTrackItem/AccountChip anatomy, tokens, states, open question resolved); Microcopy correctly empty, no ux-writer spawned — nav labels are unchanged wording, dispatcher's call, confirmed against shipped code; Accessibility 3 blocking findings, all independently verified fixed by design-lead reading the shipped `Navbar.tsx` directly (avatar-initial contrast → `brand-primary-pressed`, desktop `aria-label`s present, keyboard-operable disclosure with Escape-returns-focus). Promoted 2 design-system additions: `size.hitTarget.min` (44px) token + "Pill Track" pattern (extends principle #3). One staleness flagged, not fixed here: ui-designer's `AccountChip` anatomy still names the chip text `FirstName`/"first token of user.name" — engineering discovered mid-build that `user.name` is a household name, not a person's, and reverted the chip to show the full name (commit 1bd5833); a data-model fact outside design's original knowledge, tracked as an engineering follow-up, not a design defect.
