...
## Review
- [ ] [[features/feed-skeleton-loading]] @design — design-lead aggregation. ui-designer's 2 flagged mock-vs-real discrepancies (avatar bone 32px-in-mock vs real 40px `Avatar size="sm"`; reaction-pill bone count 2–3-in-mock vs real fixed 5 `REACTION_TYPES`) independently re-verified fixed by reading the shipped `AnnouncementCardSkeleton.tsx` directly (`h-10 w-10` avatar at line 53, 5-entry `REACTION_PILL_WIDTH_CLASSES` rendered unconditionally at lines 12/71–73) — both correct. a11y-auditor's 2 original blocking findings (aria-busy attachment point, missing prefers-reduced-motion) confirmed fixed in shipped code; 1 new non-blocking finding (roleless `<div>`+`aria-label` in `AnnouncementDetail.tsx`) confirmed fixed with `role="group"` by reading the file directly (line 50) — 0 blocking remain, 9 total findings. Microcopy correctly N/A — no ux-writer spawned, skeleton bones carry no text. Design-lead sign-off recorded on the one open call: shipped `animate-pulse` (flat fill, no gradient) vs. the mock's literal shimmer-sweep keyframe — ruled acceptable as shipped, no fast-follow, since the AC text says "pulse/shimmer" (either satisfies it) and pulse avoids introducing the mock's raw non-token `#ebe1d2` literal entirely; declined to promote ui-designer's proposed "Skeleton Shimmer" pattern into `standards/design-system.md` since the shipped code doesn't use it. No token changes to promote (zero new color tokens confirmed, matches mock/system exactly). All 3 design subsections consistent with each other and with the shipped code.
