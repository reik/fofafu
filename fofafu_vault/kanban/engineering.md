...
## Review
- [ ] eng-infra-2 [[features/migrate-render-to-vercel-supabase]] @engineering — schema translation done: `supabase/migrations/20260711000000_initial_schema.sql` — all 8 sqlite tables ported (families/announcements/comments/reactions/messages/availability_slots/playdate_requests/coach_events); `users`/`email_tokens`/`password_reset_tokens` dropped (fold into Supabase `auth.users`, resolves eng-infra-4's auth-ownership question); TEXT ids→uuid, INTEGER bools→boolean, TEXT timestamps→timestamptz; RLS enabled on all tables, policies deferred to eng-infra-4/5/6; not yet applied to a live project (no Supabase project exists) — verification blocked on account creation
