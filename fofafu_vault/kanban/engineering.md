...
## Review
- [ ] eng-backend-19 [[features/announcements-persistence-supabase]] @engineering — better-sqlite3 → pg.Pool (Supabase Postgres) migration; root cause confirmed (Render ephemeral disk); asyncHandler added to close opaque-500 failure mode; backend-dev spawn interrupted twice (harness errors), dispatcher manually ported + independently verified diff #manual-completion; 147/147 pass vs disposable Postgres, tsc clean; 0 code-level must-fix in review (1 non-blocking process gap: CI has no Postgres service container yet); manual follow-ups tracked (run migration against real Supabase project, set DATABASE_URL on Render, wire CI, confirm prod POST /api/announcements)
