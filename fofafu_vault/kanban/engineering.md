...
- [ ] eng-infra-8 [[features/migrate-render-to-vercel-supabase]] @engineering — Vercel deploy: project already exists (fofafu-frontend.vercel.app); confirm build config + env vars pointed at Supabase; staging cutover, full test suite (vitest + playwright) green against new stack, then decommission Render
...
## In Progress
- [ ] eng-infra-4 [[features/migrate-render-to-vercel-supabase]] @engineering — auth: DB side done (on_auth_user_created trigger auto-creates families row on signup; RLS policies for all 8 tables, verified live against rlizubjugevyxsfzmpny via supabase db push/migration list) — supabase/migrations/20260711010000_auth_trigger_and_rls.sql; addressed 2 security-review findings (missing WITH CHECK on UPDATE policies enabling authorship/content spoofing on messages/announcements/comments/playdate_requests, fixed via WITH CHECK + column-level GRANTs). Remaining: swap frontend auth calls from Express /auth/* to supabase-js signUp/signInWithPassword/resetPasswordForEmail/updateUser; remove backend/src/controllers/auth.controller.ts + auth.routes.ts; forced password reset for existing users

## Review
- [ ] eng-infra-5 [[features/migrate-render-to-vercel-supabase]] @engineering — Edge Functions batch 1 deployed and live-verified: family, community, search, announcement (`supabase/functions/{family,community,search,announcement}/index.ts` + shared `_shared/client.ts`); deployed via `supabase functions deploy`, smoke-tested `GET /functions/v1/search` returns 200; added `supabase/migrations/20260711020000_family_location.sql` (denormalized city/state onto families — sqlite's users-join for search no longer exists since users folded into auth.users, not queryable via PostgREST); reaction aggregation and author-name joins done as batched follow-up queries in the function since PostgREST can't replicate the sqlite LEFT JOIN/GROUP BY in one call. Not yet wired: frontend still calls the old Express endpoints, not these functions
