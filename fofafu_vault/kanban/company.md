...
## Review
- [ ] [[features/announcements-persistence-supabase]] — better-sqlite3 → Postgres (pg.Pool, Supabase-hosted) migration fixing Render ephemeral-disk 500s on POST /api/announcements; backend 147/147 vs disposable Postgres, tsc clean, 0 must-fix code review; backend-dev spawn failed twice (#manual-completion by dispatcher after bounded retry, independently re-verified); manual follow-ups remain (real Supabase migration run + Render DATABASE_URL + CI Postgres service + prod verification) tracked as open ACs, not blocking review
