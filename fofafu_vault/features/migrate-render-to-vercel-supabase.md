...
eng-infra-6/7 (2026-07-13): triggered by debugging a live `/login` redirect-loop bug on `fofafu-frontend.vercel.app` — root cause traced to (a) Vercel's `VITE_SUPABASE_URL` pointed at an unrelated Supabase project, and (b) `client.ts`'s 401 handler on the still-Express-backed `messages` route only clearing the local auth mirror, not the real Supabase session, letting a rejected token resurrect itself on reload. Fixed live (PR #49), then the user asked to complete the remaining migration tickets rather than leave the Express dependency in place.

**eng-infra-6** — ported `message`, `playdates`, `coach` Express controllers to `supabase/functions/{message,playdates,coach}/index.ts`, same pattern as batch 1 (`_shared/client.ts`, RLS-scoped per-request client). RLS (from eng-infra-4) already covers ownership; app-level rules RLS can't express — busy-slot hiding from non-owners, "can't request your own slot", "no duplicate pending request", "request already resolved", coach's rate-limit/cost-cap/holdback — are replicated in the functions. `frontend/src/api/messages.ts` and `playdates.ts` repointed at `edgeRequest`. `coach` has no frontend consumer yet (reply-coach-live's composer wiring isn't built), so only the function was ported — nothing to repoint. Deployed via `supabase functions deploy message playdates coach` and verified live against `rlizubjugevyxsfzmpny`.

Caught a real security bug via the automated post-commit review: `message/index.ts`'s `GET /threads/:userId` interpolated the URL's `partnerId` segment directly into a PostgREST `.or()` filter string — a crafted id containing filter-DSL characters (comma/paren/dot) could inject additional OR/AND clauses and read another user's messages. Fixed with a UUID-format check before the value reaches the filter string (same class of defense `family/index.ts` already uses via parameterized `.eq()` instead of `.or()`), redeployed.

Also found and fixed a latent bug (pre-existing since eng-infra-5, not introduced here): six components consuming the already-migrated `announcements.ts`/`family.ts` (`AnnouncementComposer`, `AnnouncementEditForm`, `CommentForm`, `CommentEditForm`, `FamilyEditForm`, `FamilyView`) were still checking `err instanceof ApiError` (the old Express error class) in their mutation `onError` handlers, but `edgeRequest` throws `EdgeApiError` — so specific server error messages were silently falling back to generic strings ("Could not post.", etc.) for every Edge-Function-backed mutation. Same bug newly introduced by this ticket's `messages.ts`/`playdates.ts` repoint (`MessageComposer`, `SlotForm`, `RequestCard`) was fixed the same way. All nine switched to `EdgeApiError` from `@/api/edgeClient`.

**eng-infra-7** — uploads moved from local-disk (Express + multer, `backend/src/services/uploads.service.ts`) to a Supabase Storage bucket (`supabase/migrations/20260713000000_uploads_storage_bucket.sql`). Public bucket, not signed URLs, deliberately: the old Express endpoint already served every uploaded file with no ACL check on read (only the POST was authenticated), and `mediaUrl`/`avatarUrl` columns store a permanent URL string that a private+signed approach would eventually 403 on old posts without also migrating those columns to store storage paths instead of URLs — no confidentiality requirement is being relaxed by staying public. RLS restricts writes to the caller's own `<uid>/` prefix. `frontend/src/api/uploads.ts` uploads directly via `supabase-js`'s `storage.from('uploads').upload()` instead of a multipart POST through Express. Migration applied to `rlizubjugevyxsfzmpny` and verified via `supabase migration list`.

**eng-infra-8 (partial)** — `frontend/src/api/client.ts` (the old Express fetch wrapper) had zero remaining importers once the above landed, so deleted it as dead code. That completes the frontend cutover for every domain except `coach` (unwired, no consumer to cut over). Not done: `e2e/playdates.spec.ts` still seeds test data via a direct fetch to the Express backend (`http://localhost:4100/api/auth/login`) — unrelated to `client.ts`'s removal since it never went through that file, but still a Render dependency that needs migrating before Render can be decommissioned. Per explicit instruction, Render itself is being kept alive as a rollback fallback for now, not deleted.

**eng-infra-8 (continued, 2026-07-13)** — triggered by the user reporting seeded dummy data wasn't visible in the Supabase dashboard: root cause was `backend/scripts/seed-dummy.ts` never having been repointed off the dead local sqlite file when the data layer moved to Postgres (it predates this feature). Rewrote it to use `supabaseAdmin()` — `auth.admin.createUser` (fires the `handle_new_user` trigger, which creates the `families` row) then backfills `bio`/`kid_count` and inserts `announcements` directly. Same gap existed in `e2e/playdates.spec.ts`: it was seeding via a direct POST to the Express backend on `:4100`, but the playdates UI has run on the `playdates` Edge Function since eng-infra-6 above — the spec was writing to a database the app never reads. Repointed `getToken()`/`createSlot()` at Supabase Auth REST (`/auth/v1/token?grant_type=password`) and the deployed Edge Function. Also removed dead plumbing left over from the pre-migration hybrid state now that nothing calls it: `playwright.config.ts`'s Express+sqlite `webServer` entry (and its `e2e:setup`-driven `e2e.db`), `vite.config.ts`'s `/api`/`/uploads` dev-server proxy, `VITE_API_URL` (env + `vite-env.d.ts` type). Backend's `e2e:setup` npm script renamed to `seed:dummy`, dropped its now-irrelevant sqlite `migrate` step. Verified: backend `npm run test:run` 147/147, frontend `npx vitest run` 132/132, both `tsc --noEmit` clean. PR #54 (draft).

Found and flagged, not fixed (credentials, not code): `backend/.env`'s `SUPABASE_URL` currently holds what looks like a secret-key-shaped value instead of a URL, and the service-role key is stored under `SUPABASE_SECRET_KEY` instead of the `SUPABASE_SERVICE_ROLE_KEY` name `supabaseAdmin.ts` expects — this blocked a live end-to-end run of the reseeded script and the e2e suite here, and likely affects other Supabase-Auth-dependent paths. Left for the user to correct locally; recommended rotating the exposed value as a precaution since a fragment was echoed during debugging.

Still open on eng-infra-8: staging cutover confirmation as an explicit gate, Express `auth.controller.ts`/`auth.routes.ts`/route cleanup (deliberately left in place — see eng-infra-4's "Resolution" above for why removing it prematurely was a production regression), and Render decommission itself (kept alive as rollback fallback per explicit instruction).

Backend 147/147 (unmodified — Express routes for `message`/`playdates`/`uploads`/`coach` are left in place, unused by the frontend now but still reachable, same non-deletion pattern eng-infra-4/5 established). Frontend 132/132, `tsc --noEmit` clean, `vite build` clean. PR #50 (draft, not yet merged).
