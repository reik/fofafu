...
**Follow-up (2026-09-07, closing qa-engineer's Gap #2):** fixed `ReportModal.tsx`'s `category` schema with `z.preprocess` so RHF's `null` (unselected native radio at submit time) maps to `undefined` before `z.enum` runs, letting `required_error` fire instead of falling through to Zod's raw enum message — the intentionally-failing test in `ReportModal.test.tsx` now passes with no changes to the test itself. `npm run test:frontend` — 213/213 pass. `npm run typecheck --workspace frontend` and `npm run build --workspace frontend` — clean.
...
**Spec:** `frontend/e2e/moderation-report-block.spec.ts` — all 5 scenarios qa-engineer named in `### Test plan` are implemented, one `test.describe` per scenario (scenarios 3-5 share one `test.describe.serial` block since they share a single block/unblock lifecycle by design — see below).

| Scenario (qa-engineer's naming) | Spec | Status |
|---|---|---|
| `report-flow-per-surface` — report a feed post, a comment, and a DM message, each with a category (+ an optional note on one), each confirmed via the post-a11y-fix `role="status"` region; also asserts the reachable "submit disabled until a category is picked" path | `frontend/e2e/moderation-report-block.spec.ts` (`report-flow-per-surface`) | pending (no live Supabase credentials in this sandbox) |
| `block-from-profile-and-undo` — one-tap block from the family profile page, no confirmation dialog, `BlockUndoStrip`'s inline Undo reverses it in the same visit | `frontend/e2e/moderation-report-block.spec.ts` (`block-from-profile-and-undo`) | pending (same) |
| `block-from-post-and-reload` — block from a feed post's kebab menu shows `BlockedContentPlaceholder` (with keyboard focus + Undo) in place of the row; a real `page.reload()` proves the post stays gone via server-side filtering, not just the optimistic placeholder | `frontend/e2e/moderation-report-block.spec.ts` (1st test, serial block) | pending (same) |
| `blocked-family-invisibility-sweep` — blocked family's feed post, thread comment (alongside an unblocked family's comment on the same thread, staying visible), and community-search hit all disappear; their DM thread stays in the inbox with full history and an enabled Variant-B composer | `frontend/e2e/moderation-report-block.spec.ts` (2nd test, serial block) | pending (same) |
| `unblock-restores-visibility` — unblock from the profile page; feed post and search result both reappear on a fresh navigation (not just optimistic UI) | `frontend/e2e/moderation-report-block.spec.ts` (3rd test, serial block) | pending (same) |

**Why "pending" and not "pass."** `npx playwright test e2e/moderation-report-block.spec.ts` fails at file-load time with `Missing VITE_SUPABASE_URL or VITE_SUPABASE_ANON_KEY` — this worktree has no `frontend/.env` (only `.env.example`), and none of the required env vars are set in the shell. This is the exact same, already-known gap `### E2E coverage`-equivalent notes for `[[features/header-nav-redesign]]` and `[[features/playdates]]` already document (both specs need the same two env vars for the identical reason: setup talks to the real deployed Supabase project's Auth REST + Edge Functions, there's no local backend to substitute). Confirmed empirically rather than assumed: ran `npx playwright test e2e/header-nav-redesign.spec.ts e2e/playdates.spec.ts` in this same worktree and got the identical `Missing VITE_SUPABASE_URL...` error at the identical line-shape in each file — this is a pre-existing environment gap, not something introduced by this spec.

**Corroborating evidence used instead**, per this role's instruction not to skip silently: `npm run test:frontend` (from repo root) — **212/213 pass, 1 known failure**: `ReportModal.test.tsx`'s intentionally-red "shows the 'choose a category' field error if the form is ever submitted without one" test, exactly the one qa-engineer's `### Test plan` Gap #2 already documents and left failing on purpose (the category-required Zod message is unreachable via mouse/keyboard — the submit button is disabled exactly when it would fire). Re-ran it live in this session; the count and the single failure match qa-engineer's claim exactly, with no new failures. Combined with `npx tsc --noEmit` run directly against this spec file (ad hoc, since `e2e/**` isn't in `frontend/tsconfig.json`'s `include` and so isn't part of the project's own `npm run typecheck` gate) — clean aside from two `string | undefined` narrowing notices on the top-level `SUPABASE_URL`/`SUPABASE_ANON_KEY` guard, which `header-nav-redesign.spec.ts` produces identically under the same ad hoc check (a pre-existing TS control-flow-narrowing limitation across a module-level guard into functions declared below it, not a real runtime risk — the guard throws before any test executes).

**Findings surfaced while writing this spec, not fixed here (writer ownership is `frontend/e2e/**/*.spec.ts` + this subsection only):**
- Every `ModerationMenu` trigger on a page shares the identical `aria-label="More actions"` — a11y-auditor already flagged this as non-blocking (the `moderation.trigger.ariaLabel` copy slot was never filled). Confirmed it in practice: every locator in this spec has to scope through the containing `article`/`li`/message-bubble via `.filter({ hasText })` first; a bare `page.getByRole('button', { name: 'More actions' })` would be ambiguous on any page with more than one moderatable item. Not a spec bug, just corroborating that finding is real and has a genuine ergonomic cost.
- `BlockUndoStrip` / `BlockedContentPlaceholder` / the unblock acknowledgment all interpolate the real seeded `family.name` value ("The Davis Family") into their own "...the {name} family..." copy, producing a literal "...the The Davis Family family..." double-wrap in the rendered DOM. This is **not new** — `FamilyHeader.tsx` has the identical `` `The ${family.name} family` `` pattern already, predating this feature, and `community-search.spec.ts`/`profile-pages.spec.ts` already silently tolerate it via loose regex/substring assertions rather than exact strings. This spec follows the same convention for the same reason (documented in the spec file's own top-of-file comment) rather than re-litigating a pre-existing, unrelated quirk.

**Fast-follow worth flagging, not built here:** all three block-target scenarios reuse the four general-purpose `backend/scripts/seed-dummy.ts` families (chen, davis) rather than a dedicated e2e-only family, because that's the only fixture this suite has. Block is a real, persistent, account-scoped server-side action — unlike every other seeded interaction in this suite (posts, comments, messages, playdate slots), which never hides pre-existing content from a sibling spec — so a `davis@dummy.test`-blocking test carries a genuine, acknowledged (not silently ignored) race against `home-community-rail.spec.ts`'s own "Davis" avatar-click assertion if both run concurrently against the same live account. Mitigated here with `test.describe.serial` (keeps this file's own tests ordered) and an unconditional `test.afterAll` unblock (so a failed assertion never leaves the seed data blocked for a future run), but a fifth, e2e-only "always available, never asserted-visible by any other spec" dummy family would remove the risk entirely rather than just bound it. Noted in `notes` below for the tech-lead.

### Code review

**Summary.** Re-run against the actual landed diff (`435ca75`, `182e6f6`, `2c54d18`, `c4ba6fd` + e2e-test-writer's `30daaf7`/`2f38e5a`/`e4a552f`/`45cb51b`/`6080b54`; 44 files, ~4.1k lines). This is a strong, well-corroborated dispatch: every one of the five risks this review has tracked since before any code existed was independently re-verified by reading source, not by trusting subsection prose, and all five hold up clean. `reporterId`/`blockerFamilyId` are genuinely server-derived — `supabase/functions/moderation/index.ts`'s `handleRequest`/`createBlock`/`myFamilyId` never read either from the request body, and `POST /reports`/`POST /blocks`'s own tests pass bogus client values and assert they're ignored. Blocked-family filtering is genuinely server-side end to end: two RESTRICTIVE RLS policies (`announcements`, `comments`), a `FOR ALL` RESTRICTIVE policy (`messages`), and query-level `.not("id","in",...)` exclusion (`community/index.ts`, `search/index.ts`) — read the SQL/TS directly and re-ran `community/index.test.ts`/`search/index.test.ts` live, both assert the exact filter args, not just a 200. The DM policy's `blocks.created_at <= messages.created_at` clause correctly implements the resolved Open Question (history stays, only future inbound hidden) rather than the literal, superseded AC bullet. Report and block are genuinely independent — `createReport` only touches `reports`, `createBlock`/`deleteBlock` only touch `blocks`, guarded by a fake-client test that throws if either handler reaches for the other table. The category taxonomy has zero drift: `unkind`/`privacy`/`unrelated`/`other` identical across `moderation/index.ts`'s `REPORT_CATEGORIES`, `api/moderation.ts`'s Zod enum, and `ReportModal.tsx`'s labels. I also independently traced the DM-composer "Variant B" override (ui-designer's tentative Variant A → backend-dev's confirmed Variant B, tied to contract E) and it's sound: `MessageThread.tsx`'s `MessageComposer` and `FamilyView.tsx`'s "Message this family" link both render unconditionally regardless of block state, matching the RLS policy's actual behavior (`auth.uid() <> receiver_id` always passes for the sender). Quality gates: `tsc --noEmit` (frontend) clean; `deno check` clean on all four touched/new functions (`moderation`, `community`, `search`, `message`) — `announcement/index.ts` surfaces 7 pre-existing `TS2345` errors that I confirmed, via a throwaway `git worktree` at `master`, already exist there untouched by this diff (not chargeable here). Ran the full suites live rather than trusting claimed counts: backend `deno test` is **50/50** (not the "49/49" headline in `### Backend`/`### Test plan` — see nice-to-have), frontend `npm run test:frontend` is **212 passed / 1 failed** (213 total), the one failure being a real, already-diagnosed bug (below). Two must-fix items, both test-suite-integrity issues rather than product-behavior bugs — the actual reports/blocks/RLS implementation itself is solid.

**Must-fix**
- `frontend/src/features/moderation/components/ReportModal.tsx:28` — the category-required Zod validation (`required_error: 'Choose a category to continue.'`) never fires on a real submit: React Hook Form supplies `null` (not `undefined`) for an unselected native radio-group field, and Zod's `required_error` only triggers on `undefined`, so the field falls through to the raw enum message ("Expected 'unkind' | 'privacy' | 'unrelated' | 'other', received null"). Confirmed live: `npm run test:frontend` → 212 passed, **1 failed**, at `ReportModal.test.tsx:119` (`fireEvent.submit(form)` line 130, assertion line 132) — qa-engineer's own intentionally-red regression test (`### Test plan` Gap #2), left failing on purpose per this repo's TDD convention. It's genuinely unreachable via mouse/keyboard today (Send stays `disabled` with nothing selected), but two things push this to must-fix: (1) it's a real defense-in-depth bug that would surface a confusing raw validation string to a screen-reader user the moment this path *is* reachable, and (2) as shipped, the standard `npm run test:frontend` command exits non-zero right now — the ordinary "tests are green" signal isn't true for this branch. qa-engineer's suggested fix is already correct and trivial (`z.preprocess((v) => v ?? undefined, z.enum(REPORT_CATEGORIES, { required_error: ... }))` or an explicit `.nullable()` + `.refine()`). Tech-lead's call whether this gates `review → shipped`, but the suite should be fully green (or this test explicitly marked as a tracked expected-failure rather than silently red) before that transition.
- `frontend/e2e/moderation-report-block.spec.ts:378-429` (`test.describe.serial` block) — the block/unblock lifecycle against `davis@dummy.test` (blocked by `anderson@dummy.test`) races a *concrete, already-existing* spec, not a hypothetical one. `frontend/e2e/home-community-rail.spec.ts:6,22,32` also logs in as `anderson@dummy.test` and asserts `'The Davis Family'` is visible in Anderson's own community rail, then navigates to Davis's profile heading — exactly the surface this feature's community-exclusion filter (`community/index.ts`) hides Davis from for the ~3-test duration this block stays active. `frontend/playwright.config.ts:6` sets `fullyParallel: true` with no `workers: 1` override, so on any multi-worker run these two spec *files* can execute concurrently and `home-community-rail.spec.ts`'s Davis-avatar test would flake — not from a real bug, from this block window. e2e-test-writer already flagged this honestly as an acknowledged, not-eliminated risk and proposed a dedicated e2e-only fixture family as the real fix; I agree the `test.describe.serial` + unconditional `afterAll` unblock (verified present and unconditional) only bounds *intra-file* ordering, not *inter-file* concurrency, so it doesn't actually close the gap. Mitigating factor I verified independently: `.github/workflows/ci.yml` runs neither `deno test` nor `playwright test` today (only `npm run typecheck`/`npm test --workspaces`), so this isn't causing CI failures yet — not blocking this feature's merge on it. But it's a guaranteed intermittent-failure generator the moment Playwright *is* wired into CI, which with two full specs now written purely to be run eventually is a "when," not an "if." Recommend "add a dedicated e2e-only seed family" be a required fast-follow gating e2e's CI wiring, not an optional someday-item — see `notes`.

**Nice-to-have**
- `### Backend` / `### Test plan` both cite "49/49" `deno test` passing (20 admin + 5 community + 20 moderation + 4 search). Live re-run today: **50/50** — `moderation/index.test.ts` has 21 `Deno.test(` declarations, not 20 (qa-engineer's own "New tests added this pass" round-trip test is real and passing, it's just not folded back into the summary table's headline number). Nothing broken, just a stale count worth a one-line correction next time either section is touched.
- `frontend/src/features/moderation/` has no barrel `index.ts` anywhere (feature root or per-component), so every consumer deep-imports (`@/features/moderation/components/ModerationMenu`, etc.), against the global "no deep imports, use barrel `index.ts`" rule. Matches this codebase's own dominant pattern, though — `features/feed/`, `features/messages/`, `features/family/`, `features/auth/` all skip barrels too; only `features/playdates/` has them. Systemic gap, not specific to this PR.
- `supabase/functions/moderation/index.ts:87,99` (`toReportDTO`/`toBlockDTO`) — `row: any`, each with an explicit `// deno-lint-ignore no-explicit-any`. Matches this Supabase-functions codebase's existing convention exactly (`announcement/index.ts`'s `toAnnouncementDTO`/`toCommentDTO`, `message/index.ts`'s `namesFor`, both untouched by this diff, use identical untyped `any` with no lint-ignore comment at all) — this file is actually slightly more disciplined than the pattern it's copying. A shared row-typing helper across `supabase/functions/` would be a good repo-wide follow-up, not this feature's alone to fix.
- `supabase/functions/message/index.ts` and `announcement/index.ts` still run inline in `Deno.serve(async (req) => {...})` rather than exporting a testable `handleRequest`, unlike the four functions this dispatch touched (`moderation`, `community`, `search`, pre-existing `admin`) — confirmed via grep, not just the claim. Concur with qa-engineer's own scoping call to leave this as a follow-up rather than force a source change into a feature whose own touch to these two files is doc-comments-only: the new security-relevant behavior added here (block-aware DM visibility) was independently verified via qa-engineer's live-Postgres probe, so the gap is coverage of pre-existing routing logic, not of anything this feature risked regressing. Still worth tech-lead scheduling soon, since `message/index.ts` now carries genuinely block-enforcement-relevant behavior with zero routing-layer automated coverage.
- `frontend/src/features/moderation/components/ModerationMenu.tsx:70` — every trigger's `aria-label="More actions"` is identical across an entire feed/comment list/thread (the reserved `moderation.trigger.ariaLabel` copy slot was never filled). Already flagged non-blocking by a11y-auditor and independently rediscovered by e2e-test-writer (who had to scope every locator through a containing element to disambiguate). Concur it's low-priority, but real — worth filling on a future pass.

**Acceptance criteria spot-check**
- [x] Report flow available on every announcement, comment, and DM (icon in the existing action row, not a new surface) — `ModerationMenu` wired into `AnnouncementCard.tsx`, `CommentList.tsx`, `AnnouncementDetail.tsx`, `MessageBubble.tsx`; Report item always renders regardless of target type/author state (`ModerationMenu.tsx:86-96`); confirmed live via `ModerationMenu.test.tsx` and the passing portion of `ReportModal.test.tsx`.
- [x] Report categories short + foster-family-appropriate, not generic spam/abuse/other — four categories (`unkind`/`privacy`/`unrelated`/`other`) landed identically across ux-writer's Microcopy, `moderation/index.ts`'s `REPORT_CATEGORIES`, and `api/moderation.ts`'s Zod enum; zero drift confirmed by direct comparison, not by trusting the "reconciled" claim.
- [x] Reports persist `{reporterId, targetType, targetId, category, note?, createdAt}`, admin-queue-readable — `reports` table (migration) and `toReportDTO` match exactly; `reporterId`/`createdAt` confirmed server-derived in the actual handler code, not just the test; admin `FOR ALL` RLS policy present, same shape as every other admin-visible table — reviewed by text (no live admin queue exists yet to exercise it against, which is expected per `## Out of scope`).
- [x] Block flow available on the family profile page and from any post/comment by that family — `FamilyProfileBlockControl` wired into `FamilyView.tsx`; `ModerationMenu`'s Block item wired into every card/comment/detail-page surface, correctly omitted for DMs and removed authors per spec.
- [x] Blocked family invisible to blocker: feed/comments/search vanish, DM threads list does not (per the resolved Open Question, which supersedes this bullet's literal DM wording) — all four surfaces confirmed server-side, not client-only, by reading the SQL/TS directly and re-running `community/index.test.ts`/`search/index.test.ts` live. DM history staying visible while only future inbound is hidden is correctly implemented and documented at every layer (migration comment, backend contract note, frontend Variant B, e2e assertion) — flagging explicitly so "does not vanish" isn't misread as a gap against the literal AC text.
- [x] Block is one-way; blocked family not notified — sender's `INSERT` always succeeds silently (`auth.uid() <> receiver_id` term in the messages RESTRICTIVE policy), no error surfaced; `blocks`' only non-admin RLS policy is scoped to `blocker_family_id`, so the blocked family has no read path to discover the block; the same `FOR ALL` policy also gates the receiver's mark-read `UPDATE`, so a blocked sender can't infer "seen" from a read receipt either.
- [x] Reporting and blocking are independent actions — `createReport` only touches `reports`; `createBlock`/`deleteBlock` only touch `blocks`; verified both by reading the handler code directly and by re-running the dedicated fake-client regression test (`moderation/index.test.ts:353`, "a report and a block never touch each other's table in the same request") live.
